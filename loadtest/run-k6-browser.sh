...
exec docker run --rm -i \
  ...
  -e LOAD_MOBILE_MOCK_BASE_URI="${LOAD_MOBILE_MOCK_BASE_URI:-}" \
  -e LOAD_USER_PREFIX="${LOAD_USER_PREFIX:-load-user-}" \
  -e LOAD_PASSWORD="${LOAD_PASSWORD:-load-test}" \
  -e LOAD_TEST_MODE="${LOAD_TEST_MODE:-login}" \
  -e LOAD_USER_COUNT="${LOAD_USER_COUNT:-40}" \
  -e LOAD_RATE_PER_SECOND="${LOAD_RATE_PER_SECOND:-10}" \
  -e LOAD_LOGIN_RATE_PER_SECOND="${LOAD_LOGIN_RATE_PER_SECOND:-${LOAD_RATE_PER_SECOND:-10}}" \
  -e LOAD_ENROLLMENT_RATE_PER_SECOND="${LOAD_ENROLLMENT_RATE_PER_SECOND:-${LOAD_RATE_PER_SECOND:-10}}" \
  ...
  -e LOAD_LOGIN_PRE_ALLOCATED_VUS="${LOAD_LOGIN_PRE_ALLOCATED_VUS:-${LOAD_PRE_ALLOCATED_VUS:-40}}" \
  -e LOAD_LOGIN_MAX_VUS="${LOAD_LOGIN_MAX_VUS:-${LOAD_MAX_VUS:-40}}" \
  -e LOAD_ENROLLMENT_PRE_ALLOCATED_VUS="${LOAD_ENROLLMENT_PRE_ALLOCATED_VUS:-${LOAD_PRE_ALLOCATED_VUS:-40}}" \
  -e LOAD_ENROLLMENT_MAX_VUS="${LOAD_ENROLLMENT_MAX_VUS:-${LOAD_MAX_VUS:-40}}" \
  -e LOAD_LOGIN_USER_COUNT="${LOAD_LOGIN_USER_COUNT:-${LOAD_USER_COUNT:-40}}" \
  -e LOAD_ENROLLMENT_USER_COUNT="${LOAD_ENROLLMENT_USER_COUNT:-${LOAD_USER_COUNT:-40}}" \
  -e LOAD_LOGIN_USER_OFFSET="${LOAD_LOGIN_USER_OFFSET:-0}" \
  -e LOAD_ENROLLMENT_USER_OFFSET="${LOAD_ENROLLMENT_USER_OFFSET:--1}" \
  -e LOAD_SETUP_TIMEOUT="${LOAD_SETUP_TIMEOUT:-10m}" \
  ...
  -e LOAD_SKIP_ENROLLMENT_USER_PREP="${LOAD_SKIP_ENROLLMENT_USER_PREP:-false}" \
